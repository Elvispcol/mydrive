...
export default function ChecklistForm({
  plantilla, vehiculoId, usuarioId, orgId, regionId,
}: {
  plantilla: Plantilla
  vehiculoId: string
  usuarioId: string
  ...
  const [respuestas, setRespuestas] = useState<Record<string, Respuesta>>(
    Object.fromEntries(plantilla.checklist_item.map(i => [i.id, { aprobado: null, nota: '' }])),
  )
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Guardar respuestas
    await supabase.from('preoperacional_respuesta').insert(
      items.map(i => ({
    ...
    // Si hay ítems fallados, llamar Edge Function para crear novedades
    ...
    <form onSubmit={handleSubmit} className="space-y-3">
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div className="px-4 py-3 border-b border-gray-100">
          ...
          <p className="text-xs text-gray-400">{items.length} ítems · {items.filter(i => i.critico).length} críticos</p>
        </div>
        ...
        <div className="divide-y divide-gray-100">
          {items.map(item => {
            ...
            return (
              <div key={item.id}
                className={`px-4 py-3 ${fallo ? 'bg-red-50' : ''}`}>
                <div className="flex items-center gap-3">
                  <div className="flex gap-1.5 shrink-0">
                    <button type="button" onClick={() => setAprobado(item.id, true)}
                      className={`w-8 h-8 rounded-lg text-sm font-bold transition-colors ${
                        resp.aprobado === true
                          ? 'bg-green-500 text-white'
                          : 'bg-gray-100 text-gray-400 hover:bg-green-100'
                      }`}>✓</button>
                    <button type="button" onClick={() => setAprobado(item.id, false)}
                      className={`w-8 h-8 rounded-lg text-sm font-bold transition-colors ${
                        resp.aprobado === false
      ...
      <button
        ...
        {enviando ? 'Enviando...' : `Enviar preoperacional${!todoRespondido ? ` (${items.filter(i => respuestas[i.id]?.aprobado === null).length} sin responder)` : ''}`}
      </button>
    </form>
  )
}
